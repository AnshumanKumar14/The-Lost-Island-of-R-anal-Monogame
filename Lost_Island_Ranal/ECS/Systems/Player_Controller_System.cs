...
namespace Lost_Island_Ranal.ECS
{
    ...
    {
        private readonly Equipable Knife = new Equipable(Equipable.Equipment_Type.WEAPON) {
            Run_Animation_ID = "player-knife-run",
            Idle_Animation_ID = "player-knife-idle",
            Use_Animation_ID = "player-knife-attack-1",
            ID = "Knife"
        };
        ...
        public override void Load(Entity entity)
        {
            ...
            physics.Callback = (self, o) =>
            {
                ...
                if (o.Has(Types.Item))
                {
                    ...
                    if (item.Can_Collect)
                    {
                        //var inv = (Inventory)entity.Get(Types.Inventory);
                        if (o.Has_Tag("Coin"))
                        {
                            //inv.Add_Item(o);
                            o.Destroy();
        ...
        public override void Update(GameTime time, Entity entity)
        {
            ...
            if (player.Can_Move && player.State != Player.Action_State.DASHING)
            {
                ...
                //set up player movement up down left right - can use keyboard and gamepad 
        ...
        public override void Draw(SpriteBatch batch, Entity entity)
        {
            ...
            Vector2 health_pos = camera.Screen_To_World(new Vector2(
                LostIslandRanal.ScreenWidth - (16 * health.Amount * LostIslandRanal.SCALE) - (margin * health.Amount * LostIslandRanal.SCALE), 
                16
                ));
