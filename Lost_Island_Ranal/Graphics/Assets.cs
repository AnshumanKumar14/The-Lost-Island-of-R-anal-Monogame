...
namespace Lost_Island_Ranal
{
    ...
    class Assets
    {
        ...
        public void Add_Table(string file, bool hotload = false)
        {
            ...
            {
                Console.WriteLine($"[WARNING]:: Cannot find lua file: {file}"); //Debug error, error handling --- Calls when an LUA file  cant be found
                return;
            }
        ...
        public List<Rectangle> Get_Quads(string id) {
            if (quads.ContainsKey(id) == false)
                throw new Exception("ERROR:: cannot find quads: " + id); //Debug error, error handling
            return quads[id];
        }
        ...
        public T Get <T>(string id){
            if (typeof(T) == typeof(Texture2D))
            {
                ...
                {
                    Console.WriteLine($"[WARNING]:: Cannot find lua table: {id}"); //Debug error, error handling
        ...
        public void Update(GameTime time)
        {
            ...
            foreach(var table_asset in lua_tables)
            {
                if (table_asset.Value.Hotload)
                {
                    ...
                    {
                        Console.WriteLine($"[ASSETS]::[HOTLOADER]:: Reloaded Table: {table_asset.Value.Path}"); //Debug error, error handling - wtite to console 
