...
namespace Lost_Island_Ranal.Gui
{
    class Pause_Menu
    {
        ...
        public void Draw(SpriteBatch batch)
        {
            ...
            int index = 0;
            var box_width = LostIslandRanal.ScreenWidth / 3;
            
            foreach (var name in names)
            {
                var size = font.MeasureString(name) * scale;
                float margin = 16;
                float x = LostIslandRanal.ScreenWidth / 2 - (size.X / 2);
                float y = LostIslandRanal.ScreenHeight / 2 - (size.Y + margin) / 2 - ((size.Y + margin) * names.Length / 2) + (index * (size.Y + margin)) + size.Y / 2;
                ...
                if (selector == index)
                {
                    ...
                    rect,
                    new Rectangle(
                        (int)(LostIslandRanal.ScreenWidth / 2 - (box_width / 2)),
                        (int)(y + size.Y * 0.1f),
                        box_width,
                        (int)(size.Y * 0.9f)),
            ...
            batch.Draw(
                ...
                new Rectangle(
                    (int)(LostIslandRanal.ScreenWidth / 2 - (box_width / 2)),
                    LostIslandRanal.ScreenHeight / 2 - LostIslandRanal.ScreenHeight / 3,
                    box_width,
                    (int)(LostIslandRanal.ScreenHeight / 1.5f)
                    ),
                new Rectangle(0, 0, 512, 512),
