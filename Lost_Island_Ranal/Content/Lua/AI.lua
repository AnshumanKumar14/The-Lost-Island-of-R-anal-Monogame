	...
	if physics and physics.Other then
			...
			if other:Has_Tag("Player-Hit") then
				...
				if health then
					...
					print("[LUA]::Warning:: handle_player_hit function requires the entity to have a health component.") --Error handling
				end
			end
		end
end

--Bullet from the SKULK - Range
local shulk_bullet = {
...
return {
	...
	Mech_AI = function(entity, engine)
		...
		if (engine:Entity_Within("Player", body.X, body.Y, mx_dist)) then
			anim.Current_Animation_ID = "demon-attack"
			engine:Track(entity, engine:Get_With_Tag "Player", 3)
		else
			anim.Current_Animation_ID = "demon-idle"
		end
